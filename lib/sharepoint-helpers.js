...
async function matchingUriExistsInSharepointList(sp, matchingUri) {
  const res = await spGetWithRetry(sp, {
    fields: SHAREPOINT_URI_FIELD_NAME,
    where: `${SHAREPOINT_URI_FIELD_NAME} = '${matchingUri}'`,
  });
 ...
 */
async function createNewRow(sp, matchingUri, attempt = 0) {
  if (!matchingUri) {
    console.log("No matching uuid provided, skipping creating new row");
  } else {
    const newListInstructions = {};
    newListInstructions[SHAREPOINT_URI_FIELD_NAME] = matchingUri;

    await spSetReadOnlyWithRetry(sp, SHAREPOINT_URI_FIELD_NAME, false);
    await spAddWithRetry(sp, newListInstructions);
    await spSetReadOnlyWithRetry(sp, SHAREPOINT_URI_FIELD_NAME, true);
  }
}
 ...
 */
async function updateRow(sp, matchingUri, queryParams, attempt = 0) {
  if (!matchingUri) {
    console.log(
