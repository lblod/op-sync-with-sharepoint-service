...
export async function isConfigurationValid(callback) {
...
async function spAddWithRetry(list, newListInstructions, attempt = 0) {
  try {
    const res = await list.add(newListInstructions);
    return res;
  } catch (e) {
    if (mayRetry(e, attempt)) {
      attempt += 1;
      ...
      return await spAddWithRetry(list, newListInstructions, attempt);
    } else {
      console.log(`Failed sp add call for list ${JSON.stringify(list)} and instructions ${JSON.stringify(newListInstructions)}`);
      throw e;
    }
  }
}

async function spUpdateWithRetry(list, insertionInstructions, params, attempt = 0) {
  try {
    const res = await list.add(insertionInstructions, params);
    return res;
  } catch (e) {
    if (mayRetry(e, attempt)) {
      attempt += 1;
      ...
      return await spUpdateWithRetry(list, insertionInstructions, params, attempt);
    } else {
      console.log(`Failed sp update call for list ${JSON.stringify(list)}, instructions ${JSON.stringify(insertionInstructions)} and params ${JSON.stringify(params)}`);
      throw e;
    }
  }
}
