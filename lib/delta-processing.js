import { getTypes, getMatchingUris, getValuesFromPath } from "./queries";
 ...
 *                  - matchingUris : the uuids of the linked resources
    ...
    for (let i = 0; i < deltas.length; i++) {
      ...
      for (
        ...
        for (let matchingMapping of matchingMappings) {
          ...
          if (sharepointField) {
            const matchingUris = await getMatchingUris(
              config.pathToMatchingUri,
              subject,
              sharepointField
            );
            if (matchingUris && matchingUris.length > 0 && value) {
              matchingUris.forEach((matchingUri) => {
                const queryParam = {
                  matchingUri,
                  value,
                  sharepointField,
...
async function enrichDelta(delta) {
  ...
  const finalEnrichedDelta = enrichDeltaWithRelevantConfigs(
    enrichedDeltaWithTypes
  );
...
function enrichDeltaWithRelevantConfigs(delta) {
  const relevantTypes = delta.types.filter((type) =>
    CONFIG.objects.map((t) => t.type).includes(type)
  );
  if (relevantTypes) {
    let relevantConfigs = [];
    relevantTypes.forEach((type) => {
      const relevantConfig = CONFIG.objects.filter(
...
async function getValue(params, subject, mapping) {
  ...
  if (params && params.isDeletion) {
    ...
    const result = await getValuesFromPath(
      subject,
      mapping.op
    );
    if (result) {
